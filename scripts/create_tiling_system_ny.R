...
# df        <- data.frame(cbind(subtile@data[,"tileIDm"],users$Name))
# names(df) <- c("tileIDm","username")
# df$tileIDm <- as.numeric(df$tileIDm)
# table(df$username)
# subtile@data <- df
# 
# 
...
#     my_tiles <- subtile[subtile$tileIDm %in% df[df$username == username,"tileIDm"],]
...
### Assign each tile with a username
df        <- data.frame(cbind(subtile@data[,"tileIDmm"],users$Name)) # number of rows of result is not a multiple of vector length (arg 2)
...
for(username in unique(df$username)){
  ...
  rgdal:: writeOGR(obj=subtile,
                   dsn=paste(tile_dir,export_name,".kml",sep=""),
                   layer= export_name,
                   driver = "KML",
                   overwrite_layer = T)
  
  #paste(tile_dir,export_name,".kml",sep="")
...
### Export the final subset
#export_name <- str_replace_all(paste0("tiling_system_SDN_",(subtile),"_tiles_"), " ","_")

#writeOGR(obj=my_tiles,
#         dsn=paste(tile_dir,export_name,".kml",sep="_"),
#         layer= export_name,
#         driver = "KML",
#         overwrite_layer = T)
...
###Export as Kml as a one polygon (merged)
library(maptools)


kmlPolygons(PGdv="subtile",kmlFileName="test.kml",ColorMap="colo",Pname="tileIDmm",Pdescription="IDs",Kname="IDs",Kdescription="IDs",ColorTheme="COLOUR")

### Export as KML another option


kmlfile <- paste(tile_dir, "sudantile40by40km_ny.kml", sep="/")
maptools::kmlPolygons(subtile, kmlfile= kmlfile, name="Sudan",description = "Sudan", col = "red",
...
# writeOGR(obj=tiles,
#          dsn=paste(tile_dir,export_name,".kml",sep="tileIDmm"),
#          layer= export_name,
#          driver = "KML",
#          overwrite_layer = T)

# tileIDmm
# my_tiles <- tiles[tiles$tileIDmm %in% df[df$username == username,"tileIDmm"],]
# 
...
# export_name <- paste0("SDN_one_tile_")
# 
...
# Create extent and coerce to SpatialPolygons 
ee <- as( raster::extent(-718304.767,1092705.2336,966444.8713,2562384.871), "SpatialPolygons")
...
### Give the output a decent name, with unique ID
#names(sqr_df_selected@data) <- "tileIDm" 
names(r@data) <- "tileIDm20" 
names(r_cs2@data) <- "tileIDm120"
#sqr_df_selected@data$tileIDm <- row(sqr_df_selected@data)[,1]
r@data$tileIDm20 <- row(r@data)[,1]
r_cs2@data$tileIDm120 <- row(r_cs2@data)[,1]
...
bo <- merge(r, r_cs2, by.x = "tileIDm20", by.y = "tileIDm120", duplicateGeoms = TRUE)   #if TRUE geometries in x are duplicated if there are multiple matches between records in x and y
duplicated(r_cs2$tileIDm120)
duplicated(r$tileIDm20)
bo <- merge(r_cs2,r, by.x = "tileIDm120", by.y = "tileIDm20", duplicateGeoms = TRUE)   #if TRUE geometries in x are duplicated if there are multiple matches between records in x and y
