...
aoi_dir <- ("~/Sudan_BFAST/data/AOI_shp/")
bfast_dir <- ("~/Sudan_BFAST/data/bfast_result/")
...
#result <- paste0(bfast_dir,'raw_final_results_alltiles_with_fmask.tif')
#result <-"bfast_mg_with_fmask.tif"
...
#result <- paste0(bfast_dir,substr(basename(bfast_mosaic),1,nchar(basename(bfast_mosaic))-4),'_fmask_thr_mg.tif')
...
tryCatch({
  ...
  means_b2 <- cellStats(raster(bfast_mg_fmask),"mean")
  mins_b2 <- cellStats(raster(bfast_mg_fmask),"min")
  maxs_b2 <- cellStats(raster(bfast_mg_fmask),"max")
  stdevs_b2 <- cellStats(raster(bfast_mg_fmask),"sd")
